...
The `map_search`, `dial_number` and `share_text` capabilities prepare reviewed drafts from Command or the Bridge editor. Every edit clears approval; native dispatch requires an approved action, focused foreground activity and unlocked device. Both JavaScript and Kotlin validate the single-field payload and length. The dialer uses ACTION_DIAL with 3–15 digits (optional leading +), Maps uses ACTION_VIEW with an encoded `geo:0,0?q=` query, and sharing uses ACTION_SEND with text/plain through Android’s chooser. No call or message is sent automatically. Missing handlers fail visibly; successful launches record `handed_off`, which does not claim completion in the other app. Three targeted package queries are added, without new permissions. See `docs/releases/v0.15.0.md`.
